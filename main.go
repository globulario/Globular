...
func main() {
	...
	if len(os.Args) > 1 {
		...
		connect_peer_command_secret := connect_peer_command.String("secret", "", "a secret generated by the destination peer to automaticaly accept request.")
		...
		if connect_peer_command.Parsed() {
			...
			err = connect_peer(g, *connect_peer_command_address, *connect_peer_command_user, *connect_peer_command_pwd, *connect_peer_command_secret)
			if err != nil {
				log.Println(err)
			}
		}
...
func connect_peer(g *Globule, address, user, pwd, secret string) error {
	...
	// Register the peer...
	peer, key_, err := resource_client_.RegisterPeer(token, g.Mac, g.getDomain(), Utility.MyLocalIP(), string(key), secret)
	if err != nil {
		return err
	}
	...
	_, _, err = local_resource_client_.RegisterPeer(local_token, peer.Mac, peer.Domain, peer.Address, key_, secret)
