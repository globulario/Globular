...
func (globule *Globule) startCertificateAuthorityService() error {
	// The Certificate Authority
	id := string(capb.File_proto_ca_proto.Services().Get(0).FullName())
	certificate_authority_server, port, err := globule.startInternalService(id, capb.File_proto_ca_proto.Path(), false, interceptors.ServerUnaryInterceptor, interceptors.ServerStreamInterceptor)

	if err == nil {
		globule.inernalServices = append(globule.inernalServices, certificate_authority_server)

		// Create the channel to listen on admin port.
		lis, err := net.Listen("tcp", "0.0.0.0:"+strconv.Itoa(port))
		if err != nil {
			log.Fatalf("could not certificate authority signing  service %s: %s", globule.Name, err)
		}

		capb.RegisterCertificateAuthorityServer(certificate_authority_server, globule)
...
func (globule *Globule) SignCertificate(ctx context.Context, rqst *capb.SignCertificateRequest) (*capb.SignCertificateResponse, error) {

	client_crt, err := globule.signCertificate(rqst.Csr)
